...
class ConfirmedDocumentTypeQueryHttpAdapter {
    constructor({ queryService, diagnosticLogger } = {}) {
        if (!queryService || typeof queryService.get !== "function") {
            throw new Error("ConfirmedDocumentTypeQueryHttpAdapter requires queryService");
        }
    ...
    async handle({
        ...
        let result;
        try {
            result = await this.queryService.get({
                connectorId,
                credential,
                sourceDocumentKey,
                sourceUpdatedAt,
            ...
            this.diagnoseError(requestId, "confirmed_document_type_query_application_exception");
            return this.createSafeProcessingError(requestId);
        }

        if (!result || typeof result !== "object" || Array.isArray(result)) {
            this.diagnoseError(requestId, "confirmed_document_type_query_result_invalid");
            return this.createSafeProcessingError(requestId);
        }
        ...
        if (result.status === "invalid") {
            return {
                ...
                body: {
                    requestId,
                    status: "invalid",
                    errorCode: "confirmed_document_type_query_invalid"
                }
            };
        }
        ...
        if (result.status === "error") {
            const internalErrorCode =
                typeof result.errorCode === "string" && result.errorCode.trim()
                    ...
                    : "confirmed_document_type_query_error";
...
module.exports = ConfirmedDocumentTypeQueryHttpAdapter;
