...
class ConfirmedDocumentTypeQueryTransport {
    constructor({ httpAdapter, credentialTransport, connectorIdHeader = "x-risen-connector-id" } = {}) {
        if (!httpAdapter || typeof httpAdapter.handle !== "function") {
            throw new Error("ConfirmedDocumentTypeQueryTransport requires httpAdapter");
        }
        if (!credentialTransport || typeof credentialTransport.extract !== "function") {
            throw new Error("ConfirmedDocumentTypeQueryTransport requires credentialTransport");
        }
    ...
    async handle({ method, headers, query } = {}) {
        ...
        const queryKeys = query && typeof query === "object" && !Array.isArray(query)
            ? Object.keys(query)
            : [];
        const allowedKeys = new Set(["sourceDocumentKey", "sourceUpdatedAt", "sourceSize"]);

        if (queryKeys.length !== 3 || queryKeys.some(key => !allowedKeys.has(key))) {
            return { httpStatus: 422, body: { requestId, errorCode: "confirmed_document_type_query_invalid" } };
        }

        const sourceDocumentKey = typeof query.sourceDocumentKey === "string"
            ? query.sourceDocumentKey.trim()
            : "";
        const sourceUpdatedAt = typeof query.sourceUpdatedAt === "string"
            ? query.sourceUpdatedAt.trim()
            : "";
        const rawSourceSize = typeof query.sourceSize === "string"
            ? query.sourceSize.trim()
            : "";
        const sourceSize = /^\d+$/.test(rawSourceSize)
...
module.exports = ConfirmedDocumentTypeQueryTransport;
